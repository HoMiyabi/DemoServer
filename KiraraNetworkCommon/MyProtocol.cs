...
namespace Kirara.Network
{
    public static class MyProtocol
    {
        ...
        public const int MsgIdFieldSize = sizeof(uint);
        public const int RpcSeqFieldSize = sizeof(uint);
        public const int HeaderSize = MagicNumFiledSize + LenFieldSize + MsgIdFieldSize + RpcSeqFieldSize;

        public static void ReadHeader(MyBuffer buffer,
            out ushort magicNum, out int len, out uint msgId, out uint rpcSeq)
        {
            ...
            msgId = BinaryPrimitives.ReadUInt32BigEndian(buffer.Dequeue(MsgIdFieldSize));
            rpcSeq = BinaryPrimitives.ReadUInt32BigEndian(buffer.Dequeue(RpcSeqFieldSize));
        }

        private static void WriteHeader(MyBuffer buffer, int len, uint msgId, uint rpcSeq)
        {
            ...
            BinaryPrimitives.WriteUInt32BigEndian(buffer.Enqueue(MsgIdFieldSize), msgId);
            BinaryPrimitives.WriteUInt32BigEndian(buffer.Enqueue(RpcSeqFieldSize), rpcSeq);
        }

        public static byte[] GetSendBytes(uint msgId, uint rpcSeq, IMessage msg)
        {
            int len = msg.CalculateSize();
            var buffer = new MyBuffer(HeaderSize + len);
            WriteHeader(buffer, len, msgId, rpcSeq);
